...
describes what users get. Pick a format with `-o`, or let gitee choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
gitee <command> -o table   # aligned columns for reading
gitee <command> -o jsonl   # one JSON object per line, for piping
gitee <command> -o json    # a single JSON array
gitee <command> -o csv     # spreadsheet friendly
gitee <command> -o tsv     # tab-separated
gitee <command> -o url     # just the URL column
gitee <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
gitee <command> --fields id,title,url
```
...
```bash
gitee <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
gitee <command>            # a table, because this is a terminal
gitee <command> | wc -l    # JSONL, because this is a pipe
```
